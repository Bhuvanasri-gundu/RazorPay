...
### Live Demo — Scenario 2 (Repeated UPI Failure)
![Live Demo Scenario 2](docs/screenshots/live-demo-scenario-2.png)
Scenario 2 demonstrates REVA handling a repeated UPI failure case. The autonomous execution engine progresses through Detection, Gemini AI diagnosis, Policy Engine validation, Action Execution, and Outcome stages in real time, dispatching a recovery payment link when retries are exhausted.

### Razorpay Test Checkout Modal
![Razorpay Test Checkout Modal](docs/screenshots/razorpay-test-checkout.png)
When a customer opens a REVA recovery payment link, the integrated Razorpay Standard Checkout modal launches in test mode with prefilled contact details, offering card, netbanking, and UPI payment options for completing the recovered transaction.
