...
REVA is an autonomous revenue recovery agent designed to recover lost revenue from failed e-commerce and SaaS payments. Powered by FastAPI, Next.js 16, Google Gemini 2.0 Flash, a deterministic policy engine, Supabase PostgreSQL, and Razorpay Test Mode, REVA analyzes payment failures, evaluates recovery risk, enforces safety guardrails, and executes bounded recovery actions such as smart retries and personalized payment links.
...
2. **AI Diagnosis**: Google Gemini 2.0 Flash evaluates the transaction attributes (payment method, failure reason code, retry history, and customer success profile) and recommends a targeted recovery action.
...
| AI Diagnosis | Google GenAI SDK | Gemini 2.0 Flash for transaction failure analysis |
...
- **Google Gemini AI**: When `GEMINI_API_KEY` is provided, REVA dispatches structured payment failure telemetry to Gemini 2.0 Flash. When absent, an intelligent rule-based heuristic provides realistic diagnostic responses.
