...
REVA is an autonomous revenue recovery agent designed to recover lost revenue from failed e-commerce and SaaS payments. Powered by FastAPI, Next.js 16, Google Gemini 3.5 Flash-Lite, a deterministic policy engine, Supabase PostgreSQL, and Razorpay Test Mode, REVA analyzes payment failures, evaluates recovery risk, enforces safety guardrails, and executes bounded recovery actions such as smart retries and personalized payment links.
...
2. **AI Diagnosis**: Google Gemini 3.5 Flash-Lite evaluates the transaction attributes (payment method, failure reason code, retry history, and customer success profile) and recommends a targeted recovery action.
...
![Razorpay Test Checkout](docs/screenshots/razorpay-checkout-page.png)
Dynamic payment recovery links generated by REVA direct customers to the integrated Razorpay test checkout portal, displaying the amount due (₹4,999), customer details, and alternative payment method options.

### Razorpay Test Mode Payment Options
![Razorpay Test Mode Payment Options](docs/screenshots/razorpay-test-mode-options.png)
When a customer clicks to complete payment, the official Razorpay Standard Checkout SDK launches in sandbox test mode, displaying the initial contact details prompt, price summary (₹4,999), available payment options (Cards, Netbanking, UPI), and the red Test Mode indicator.
...
| AI Diagnosis | Google GenAI SDK | Gemini 3.5 Flash-Lite for sub-second failure analysis |
...
- **Google Gemini AI**: When `GEMINI_API_KEY` is provided, REVA dispatches structured payment failure telemetry to Gemini 3.5 Flash-Lite. When absent, an intelligent rule-based heuristic provides realistic diagnostic responses.
